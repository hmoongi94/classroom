...
// async와 함께 사용한 것을 통해 Promise객체를 동기적으로 작성된 코드처럼 작동합니다.
...
} catch(error){ // try구문이 실패했을 경우는 통신상태 불량일 확률이 상당히 높습니다.
  console.error('fetch error:',error)
  // 실패 시 메시지 업데이트
  element.innerHTML = "아직 데이터가 수진되지 않았습니다."
}
...
fetchExample('div',{class:'example'},'로딩중','http://my.server.localhost/directory').then((div)=>{root?.appendChild(div)}).catch((error)=>{console.error('catch구문 error 매개변수 인자가 전달됨:',error)})