...
async function fetchExample(
  ...
  try {
    ...
    if (!response.ok) {
      // AJAX 통신의 일반적인 객체 구조에서는 ok 속성을 사용한 예
    ...
    // try구문이 실패했을 경우는 통신상태 불량일 확률이 상당히 높습니다.
...
fetchExample(
  ...
  { class: "example" },
  "로딩중",
  "http://my.server.localhost/directory"
)
  .then((div) => {
    root?.appendChild(div);
  })
  .catch((error) => {
    console.error("catch구문 error 매개변수 인자가 전달됨:", error);
  });
