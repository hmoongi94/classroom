...
/**
 * @param obj User 객체
 * @returns Object를 JSON 문자열로 변환한 후 다시 JSON객체로 변환한 User객체
 */
...
/**
 * 얕은복사와 깊은복사의 차이점
 * 얕은복사는 객체의 속성이 객체일 경우, 참조만 복사하게 됩니다.
 * 깊은복사는 객체의 속성이 객체일 경우, 객체를 복사하게 됩니다.
 * 메모리 관점에서 얕은 복사는 원본 객체와 복사된 객체가 같은 객체를 참조하고 있기 때문에, 복사된 객체를 수정하면 원본객체도 수정되게 됩니다.
 * 깊은 복사는 원본객체와 복사된 객체가 다른 객체를 참조하고 있기 때문에, 복사된 객체를 수정해도 원본객체는 수정되지 않습니다.
 * 
