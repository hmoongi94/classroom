...
// console.log(_pokemon[0]) ->이상해씨
// console.log(_pokemon.length) ->905
// pokemon = _pokemon[i] -> console.log(pokemon[0]) -> 첫글자만 905개 나옴



// _pokemon[906]="새로운 포켓몬"
// console.log(_pokemon[906])


_pokemonlength= _pokemon.length+1
  for(i=0; i<_pokemonlength+1; i++){
  if(_pokemon[i]===undefined){
    _pokemon[i]= "새로운 포켓몬"
  }    
}
console.log(_pokemon[906])
// 문제 905, 906 둘다 "새로운 포켓몬"이 나옴. 해결방안은 없을까?

// 2. '_pokemon' 배열의 마지막 포켓몬을 방출해주세요. 이를 위해 배열의 마지막 원소를 제거하고, 그 원소의 이름을 출력하세요.
_pokemonlength=_pokemon.length

const outofpokemon = []

for(i=0; i<_pokemonlength; i++){
  if(_pokemon[i]===_pokemon[_pokemonlength-1]){
    outofpokemon[0] = _pokemon[_pokemonlength-1]
    _pokemon[i]=undefined
    
  }
}

console.log(_pokemon[904])
console.log(outofpokemon[0])

// const, let 차이 
// const는 데이터타입 그대로 넣어줘야함
...
for(i=0; i<_pokemon.length; i++){
  ...
  if(_pokemon[i]===findpokemon){
    ...
    if(i===_pokemon[_pokemon.length-1]){
      console.log(-1)
    }
  }
}
...
for(i=0; i<_pokemon.length; i++){
  if(findpokemon === _pokemon[i]){
  console.log(_pokemon[i])
  } else{
    if(i===_pokemon.length-1){
      console.log("이 포켓몬은 없습니다.")
    }
  }
...
for(let i=_pokemon.length-1; i>-1; i--){
  console.log(_pokemon[i])
}

// let newArray = []
// for(let i =0; i <_pokemon.length; i++) {
//   newArray[i] = _pokemon[_pokemon.length -i]

//7. '_pokemon' 배열의 원소들을 가나다 순으로 정렬해 출력해주세요.

arrayPokemon=["가","갸","거","겨","고","교","구","규","그","기","나","다","라","마","바"]

for(j=0; j<arrayPokemon.length; j++){
for(i=0; i<_pokemon.length; i++){
  if(_pokemon[i][0]===arrayPokemon[j])
  {console.log(_pokemon[i])}
}
}

// for(i=0; i<_pokemon.length; i++){
//   if(_pokemon[i][0]===arrayPokemon[1])
//   {console.log(_pokemon[i])}
// }

// for(i=0; i<_pokemon.length; i++){
//   if(_pokemon[i][0]===arrayPokemon[2])
//   {console.log(_pokemon[i])}
// }
...
listofpokemon=[]
for(i=0; i<6; i++){
  listofpokemon[i]=gacha(_pokemon)
  console.log(listofpokemon[i])
}

//9. '_pokemon' 배열과 다른 하나인 '_digimon' 배열을 연결해 새로운 배열을 생성해주세요.

const _digimon =["디지몬", "아구몬", "워그레이몬"]
let pokePlusDigimon=[]
let pokePlusDigimonLength = _pokemon.length + _digimon.length

for(i=0; i<pokePlusDigimonLength; i++){
  pokePlusDigimon[i]=_pokemon[i]
  if(_pokemon[i]===undefined){
    pokePlusDigimon[i]=_digimon[i-_pokemon.length]
  }
...
console.log(pokePlusDigimon[906])
...
while(true){
  console.log(i+1);
  i++;
  if(_pokemon[i]===undefined) break;
}
// while문 연습을 더 해보자.

//11. '_pokemon' 배열에서 특정 인덱스의 포켓몬 이름을 새로운 이름으로 변경해주세요.

let chooseNumber=[500]
let newPokemonName="앙기모띠"
// _pokemon[0]=newPokemonName
// console.log(_pokemon[0])
...
for(i=0; i<_pokemon.length; i++){
  _pokemon_lv2[i]=_pokemon[i]+"LV2"
}

console.log(_pokemon_lv2)
...
for(i=0; i<_pokemon.length; i++){
  if(_pokemon[i][0]===findletterdoo){
    countHandler=countHandler+1
    if(countHandler===2){console.log(_pokemon[i])}
  }    
}
...
for(i=0; i<_pokemon.length; i++){
  _pokemonPlusdot[i]=_pokemon[i]+","
}
// console.log(_pokemonPlusdot)

let oneString=""

  // onestring=_pokemonPlusdot[0]+_pokemonPlusdot[1]+_pokemonPlusdot[2]

for(i=0; i<_pokemonPlusdot.length; i++){
  oneString=oneString+_pokemonPlusdot[i]
}
...
chooseNumber=[50]
// console.log(chooseNumber[0])

// 17. '_pokemon' 배열을 새로운 배열에 복사해 문서에 적절한 태그로 생성하여 출력해주세요.
let _pokemon_copy=[]
_pokemon_copy=_pokemon
console.log(_pokemon_copy)

const pokemonlist= document.createElement("h3")
document.body.appendChild(pokemonlist)

pokemonlist.textContent= _pokemon_copy